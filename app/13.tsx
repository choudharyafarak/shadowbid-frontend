...
export default function ShadowBidMarketplace() {
  ...
  const init = useCallback(async () => {
    try {
      ...
      try {
        ...
        console.warn("Contract read failed");
        return;
      }
      ...
      // 3. Setup Wallet only (Defer FHEVM init until needed to avoid errors)
      if (typeof window !== 'undefined' && window.ethereum) {
  ...
  // --- UPDATED BID FUNCTION (Just-In-Time Security Init) ---
  const handleBid = async () => {
    if (!account) return setIsWalletModalOpen(true);
    if (!selectedAuction) return;

    setIsLoading(true);
    setStatus("Initializing Security...");
    ...
    try {
      ...
      // 2. Initialize FHEVM (If not ready)
      let instance = fhevm;
      if (!instance) {
        // We use standard init - no hardcoded KMS addresses
      ...
      // Add a small delay to ensure instance is ready
      ...
      const tx = await contract.bid(
        ...
        { gasLimit: 10000000 } // Force high gas limit to prevent FHE estimation errors
      );
      ...
      // If FHE init failed, reset it so we try again next time
      setFhevm(null);
    }
    setIsLoading(false);
  };
            ...
            {auctions.length === 0 ? (
              <div className="py-20 border border-dashed border-neutral-800 rounded-3xl bg-neutral-900/10 text-center relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <button onClik={() => setView('create')} className="bg-yellow-400 hover:bg-yellow-300 text-black font-bold px-8 py-4 rounded-xl transition-all cursor-pointer shadow-xl shadow-yellow-400/20 active:scale-95 flex items-center gap-2 mx-auto"><PlusCircle className="w-5 h-5"/> Create First Auction</button>
                </div>
              </div>
            ) : (
