...
// --- CONFIGURATION ---
// 1. YOUR V2 CONTRACT ADDRESS
const CONTRACT_ADDRESS = "0xc082c1cf4466308dc3fc82d0D36Ac1FB09977D83"; 

// 2. PUBLIC "READ-ONLY" CONNECTION (So images load without MetaMask)
...
export default function ShadowBidMarketplace() {
  ...
  // Get the specific auction data from ID
  ...
  // --- HYBRID INITIALIZATION (Read-Only + Wallet) ---
  const init = useCallback(async () => {
    try {
      // A. SETUP READ PROVIDER (Works for everyone)
      // If MetaMask is available, use it. If not, use Public Node.
      let readProvider;
      if (window.ethereum) {
      ...
      // B. FETCH DATA (Using the Read Provider)
      ...
      console.log("Auctions found:", count.toString());
      ...
      // C. SETUP WALLET (Only if connected)
      if (window.ethereum) {
        ...
        try {
          ...
          // Init Zama Encryption Engine
          const instance = await createInstance({ chainId: 11155111 });
          setFhevm(instance);
        } catch (e) {
          console.log("User wallet not connected yet (Browsing mode)");
        }
      }
    } catch (e) { 
          ...
          <div className="flex items-center gap-4">
            ...
            {!account ? (
              ...
              <div className="flex items-center gap-3 bg-neutral-900 rounded-full pl-1 pr-4 py-1 border border-neutral-800 cursor-pointer hover:border-yellow-400/30 transition-colors shadow-lg">
                ...
                <div className="flex flex-col"><span className="text-xs font-bold text-white leading-none">{account.slice(0,6)}...</span><span className="text-[10px] text-neutral-500 font-mono leading-none mt-1">{balance} ETH</span></div>
              </div>
            )}
          </div>
        </div>
            ...
            <div className="mt-32 pt-16 border-t border-neutral-800">
              ...
              <div className="grid md:grid-cols-3 gap-8">
                <div className="p-8 rounded-3xl bg-neutral-900/50 border border-neutral-800 hover:bg-neutral-900 transition-colors cursor-default group">
                  ...
                  <p className="text-neutral-400 text-sm leading-relaxed">Bids are encrypted on your device using FHE before they ever reach the blockchain.</p>
                </div>
                <div className="p-8 rounded-3xl bg-neutral-900/50 border border-neutral-800 hover:bg-neutral-900 transition-colors cursor-default group">
                  ...
                  <p className="text-neutral-400 text-sm leading-relaxed">The smart contract compares encrypted values mathematically without seeing them.</p>
                </div>
                <div className="p-8 rounded-3xl bg-neutral-900/50 border border-neutral-800 hover:bg-neutral-900 transition-colors cursor-default group">
                  ...
                  <p className="text-neutral-400 text-sm leading-relaxed">Runs on-chain. Anyone can verify the code execution was tamper-proof.</p>
                </div>
              </div>
            </div>
          </div>
