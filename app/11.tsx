...
export default function ShadowBidMarketplace() {
    ...
    try {
      ...
      if (typeof window !== 'undefined' && window.ethereum) {
        ...
        if (accounts.length > 0) {
          ...
          initFhevm(); // Trigger FHEVM init separately
        }
      }
    } catch (e) { 
  ...
  const initFhevm = async () => {
    try {
      ...
      // Standard init - let the library determine the best configuration
      const instance = await createInstance({ chainId: 11155111 });
      setFhevm(instance);
      setFhevmReady(true);
      setStatus("Security Layer Ready");
    } catch (e) {
      console.error("FHEVM Init failed", e);
      setFhevmReady(false);
      setStatus("Security Init Failed");
    }
  };
  ...
  const handleBid = async () => {
    ...
    // Retry FHEVM if missing
    if (!fhevm) {
      await initFhevm();
      if (!fhevmReady) {
        alert("Security layer failed to initialize. Please refresh the page.");
        return;
      }
    }
    ...
    try {
      ...
      if (!fhevm) throw new Error("FHEVM instance missing");
      ...
      const tx = await contract.bid(
        ...
        { gasLimit: 10000000 } // Force high gas limit
      );
      ...
      <nav className="border-b border-neutral-800 bg-neutral-900/50 backdrop-blur-md sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            {!fhevmReady && account && (
              <button onClick={initFhevm} className="hidden md:flex items-center gap-2 text-xs font-bold text-red-400 bg-red-900/20 px-3 py-1.5 rounded-full border border-red-500/30 hover:bg-red-900/40 transition-colors" title="Security Layer Not Ready">
                <RefreshCw className="w-3 h-3" /> Retry Security
              </button>
            )}
