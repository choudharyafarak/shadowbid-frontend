      ...
      if (window.ethereum) {
        ...
        if (accounts.length > 0) {
          ...
          try {
            ...
            setFhevm(instance);
          } catch (e) { console.error("FHEVM Init failed", e); }
        }
      }
    } catch (e) { 
  ...
  const handleBid = async () => {
    ...
    try {
      ...
      if (!instance) {
        ...
        instance = await createInstance({ chainId: 11155111 });
        setFhevm(instance);
      }

      // --- DECIMAL FIX ---
      // We scale the bid by 100 to allow 2 decimal places (e.g., 1.50 -> 150)
      ...
      // Zama euint32 max is ~4.2 Billion. 
      // If we scale by 100, max bid is ~42 Million ETH. Safe.
      ...
      // --- GAS FIX ---
      // We manually set gasLimit to 10,000,000 to bypass estimation errors
      // inherent to FHE transactions on public testnets.
      ...
      console.error(e);
      setStatus("Bid Failed. See Console.");
    }
    setIsLoading(false);
  };
      ...
      {isWalletModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in cursor-default">
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl w-full max-w-sm p-6 relative shadow-2xl">
            ...
            <div className="text-center mb-8"><div className="w-16 h-16 bg-neutral-800 rounded-2xl mx-auto flex items-center justify-center mb-4 text-yellow-400 shadow-lg shadow-yellow-400/10"><Wallet className="w-8 h-8" /></div><h3 className="text-2xl font-bold text-white">Connect Wallet</h3><p className="text-neutral-400 text-sm mt-2">Secure connection to Sepolia Network</p></div>
            <div className="space-y-3">
              <button onClick={connectWallet} className="w-full flex items-center justify-between p-4 rounded-xl bg-neutral-800 hover:bg-neutral-750 border border-neutral-700 hover:border-orange-500/50 transition-all cursor-pointer group shadow-lg"><span className="font-bold text-lg group-hover:text-orange-400 transition-colors">MetaMask</span><div className="w-10 h-10 rounded-full bg-white flex items-center justify-center p-1"><img src="https://upload.wikimedia.org/wikipedia/commons/3/36/MetaMask_Fox.svg" className="w-full h-full"/></div></button>
              <button onClick={connectWallet} className="w-full flex items-center justify-between p-4 rounded-xl bg-neutral-800 hover:bg-neutral-750 border border-neutral-700 hover:border-blue-500/50 transition-all cursor-pointer group shadow-lg"><span className="font-bold text-lg group-hover:text-blue-400 transition-colors">Browser Wallet</span><div className="w-10 h-10 rounded-full bg-blue-500/20 text-blue-400 flex items-center justify-center"><Wallet className="w-5 h-5"/></div></button>
            </div>
      ...
      {isTcOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in cursor-default">
          <div className="bg-neutral-900 border border-neutral-800 rounded-2xl w-full max-w-2xl shadow-2xl relative flex flex-col max-h-[80vh]">
            <div className="p-6 border-b border-neutral-800 flex justify-between items-center bg-neutral-900 rounded-t-2xl sticky top-0 z-10"><h3 className="text-xl font-bold flex items-center gap-2"><FileText className="w-5 h-5 text-yellow-400"/> Terms of Service</h3><button onClick={() => setIsTcOpen(false)} className="text-neutral-500 hover:text-white cursor-pointer bg-neutral-800 p-1 rounded-full"><X className="w-5 h-5" /></button></div>
                ...
                <div className="bg-neutral-900 p-8 rounded-3xl border border-neutral-800 shadow-xl relative overflow-hidden">
                  ...
                  {selectedAuction.isActive ? (
                    <div className="space-y-6 relative z-10">
                      ...
                      <button onClick={handleBid} disabled={isLoading} className="w-full bg-yellow-400 hover:bg-yellow-300 text-black font-bold py-5 rounded-2xl flex items-center justify-center gap-3 disabled:opacity-50 cursor-pointer shadow-xl shadow-yellow-400/20 active:scale-[0.98] transition-all text-lg">{isLoading ? status : <>Place Private Bid <Lock className="w-5 h-5"/></>}</button>
                    </div>
                  ) : (
