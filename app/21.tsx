...
const ZAMA_CONFIG = {
  ...
  gatewayUrl: "https://gateway.sepolia.zama.ai", // RESTORED: Critical for fetching keys
      ...
      if (typeof window !== 'undefined' && window.ethereum) {
        ...
        if (accounts.length > 0) {
          ...
          try {
             ...
             console.warn("Auto-init FHEVM failed, waiting for user action", e);
          }
        }
      }
    } catch (e) { 
  ...
  const handleBid = async () => {
    ...
    try {
      ...
      if (!instance) {
        ...
        try {
          // We MUST provide the Gateway URL so it can fetch the Public Key
          instance = await createInstance({ 
            chainId: 11155111,
            network: window.ethereum,
            kmsContractAddress: ZAMA_CONFIG.kmsContractAddress,
            aclContractAddress: ZAMA_CONFIG.aclContractAddress,
            gatewayUrl: ZAMA_CONFIG.gatewayUrl, // This is what was missing!
            relayerUrl: ZAMA_CONFIG.relayerUrl,
          });
          setFhevm(instance);
        } catch (e) {
          console.error("FHEVM Init Failed:", e);
          alert("Security Layer Failed. Please check your internet/firewall.");
          setIsLoading(false);
          return;
        }
      }
    ...
    <div className="min-h-screen bg-neutral-950 text-white font-sans selection:bg-yellow-500 selection:text-black flex flex-col relative">
      ...
      <footer className="border-t border-neutral-800 py-10 mt-12 bg-neutral-900/30">
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center text-sm text-neutral-500 gap-6">
          ...
          <a href="https://x.com/0xshinkai" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:text-white tranition-all duration-300 group cursor-pointer bg-neutral-800/50 px-4 py-2 rounded-full border border-neutral-700/50 hover:border-yellow-400/30 hover:bg-neutral-800">
            <span>Built by</span>
