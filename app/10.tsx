...
// Known Zama Gateway/KMS Address for Sepolia (Fixes the "KMS Invalid" error)
const SEPOLIA_KMS_ADDRESS = "0x9D6891A6240D6130c54aa2c1d36884360e670244";
...
export default function ShadowBidMarketplace() {
        ...
        if (accounts.length > 0) {
          ...
          try {
            ...
            setFhevm(instance);
             ...
             // Fallback with explicit KMS address if standard fails
             try {
  ...
  const handleBid = async () => {
    ...
    try {
      ...
      // Try to re-initialize if missing
      if (!instance) {
        setStatus("Initializing Zama Encryption...");
        try {
          instance = await createInstance({ chainId: 11155111 });
        } catch (e) {
          // Retry with explicit KMS address
          console.warn("Retrying FHEVM with KMS...");
