...
// --- ZAMA SEPOLIA CONFIGURATION (FROM YOUR DATA) ---
const ZAMA_CONFIG = {
  // Addresses from your provided table
  kmsContractAddress: "0xbE0E383937d564D7FF0BC3b46c51f0bF8d5C311A",
  aclContractAddress: "0xf0Ffdc93b7E186bC2f8CB3dAA75D86d1930A433D",
  // We use the Relayer URL provided
  relayerUrl: "https://relayer.testnet.zama.org", 
  // We REMOVED gatewayUrl because it was causing the "fetch error"
};
...
export default function ShadowBidMarketplace() {
    ...
    try {
      ...
      if (typeof window !== 'undefined' && window.ethereum) {
        ...
        if (accounts.length > 0) {
          ...
          // Try to Init FHEVM without forcing Gateway URL
          try {
             const instance = await createInstance({ 
               ...
               // relayerUrl: ZAMA_CONFIG.relayerUrl // Optional: Let library use defaults if this fails
             });
             setFhevm(instance);
          } catch (e) {
             console.warn("Auto-init of FHEVM failed (will retry on bid)", e);
          }
        }
      }
    } catch (e) { 
  ...
  const handleBid = async () => {
    ...
    try {
      // 1. Ensure FHEVM is ready (Retry logic included)
      let instance = fhevm;
      if (!instance) {
        setStatus("Initializing Zama Encryption...");
        try {
          // PASS EXACT CONFIGURATION FROM YOUR TABLE
          instance = await createInstance({ 
            chainId: 11155111,
            network: window.ethereum,
            kmsContractAddress: ZAMA_CONFIG.kmsContractAddress,
            aclContractAddress: ZAMA_CONFIG.aclContractAddress,
            // We use the library's default gateway logic or relayer fallback
            // Removing 'gatewayUrl' fixes the "wrong gateway url" error
          });
          setFhevm(instance);
        } catch (e) {
          console.error("FHEVM Init Failed:", e);
          alert("Security Layer Failed to Initialize. Check console.");
          setIsLoading(false);
          return;
        }
      }
