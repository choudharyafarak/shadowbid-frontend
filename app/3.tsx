...
import { Shield, Lock, Wallet, ChevronRight, Gavel, Clock, Trophy, ExternalLink, User, X, FileText, CheckCircle2, PlusCircle, Search, Share2, ArrowLeft, Trash2, Zap, EyeOff, Activity, BarChart3, Users } from 'lucide-react';

const CONTRACT_ADDRESS = "0x8Eb35980AC487A5bd799F3520e82481d0496B258";
          ...
          <div className="animate-in fade-in duration-500">
            ...
            {auctions.length === 0 ? (
              ...
              <div className="grid md:grid-cols-3 gap-8">
                {auctions.map((auc) => (
                  <div key={auc.id} onClick={() => { setSelectedAuction(auc); setView('view_auction'); }} className="group bg-neutral-900 border border-neutral-800 rounded-2xl overflow-hidden hover:border-yellow-400/50 transition-all cursor-pointer hover:shadow-2xl hover:shadow-yellow-400/10 hover:-translate-y-2 flex flex-col">
          ...
          <div className="animate-in fade-in duration-500">
            ...
            <div className="grid md:grid-cols-2 gap-12">
              <div className="rounded-3xl overflow-hidden bg-neutral-900 border border-neutral-800 h-[600px] relative shadow-2xl">
                ...
                <div className="absolute top-4 right-4"><button onClick={() => { navigator.clipboard.writeText(window.location.href); alert("Copied!"); }} className="p-3 bg-black/50 backdrop-blur-md rounded-full text-white border border-white/10 hover:bg-white hover:text-black transition-colors cursor-pointer shadow-lg group"><Share2 className="w-5 h-5"/></button></div>
              </div>
