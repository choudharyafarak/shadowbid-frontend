...
export default function ShadowBidMarketplace() {
  ...
  const handleBid = async () => {
    ...
    try {
      ...
      // Initialize FHEVM with EXPLICIT Sepolia V0.9 Addresses
      if (!instance) {
            ...
            {auctions.length === 0 ? (
              <div className="py-20 border border-dashed border-neutral-800 rounded-3xl bg-neutral-900/10 text-center relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <button onClick={() => setView('create')} className="bg-yellow-400 hover:bg-yellow-300 text-black font-bold px-8 py-4 rounded-xl transition-all cursor-pointer shadow-xl shadow-yellow-400/20 active:scae-95 flex items-center gap-2 mx-auto"><PlusCircle className="w-5 h-5"/> Create First Auction</button>
                </div>
              </div>
            ) : (
