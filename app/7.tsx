...
export default function ShadowBidMarketplace() {
  ...
  const init = useCallback(async () => {
    try {
      ...
      try {
        ...
        console.warn("Contract not found or network error. Using fallback.");
        return;
      }
  ...
  const handleBid = async () => {
    ...
    setIsLoading(true);
    setStatus("Preparing Security Layer...");

    try {
      // 1. Ensure FHEVM is ready (Auto-Initialize if missing)
      let instance = fhevm;
      if (!instance) {
      ...
      console.error(e);
      setStatus("Bid Failed or Cancelled.");
    }
    setIsLoading(false);
  };
