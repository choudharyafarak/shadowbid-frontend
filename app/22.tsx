...
import { Shield, Lock, Wallet, ChevronRight, Gavel, Clock, Trophy, ExternalLink, User, X, FileText, CheckCircle2, PlusCircle, Search, Share2, ArrowLeft, Trash2, Zap, EyeOff, Activity, RefreshCw } from 'lucide-react';
...
// --- ZAMA CONFIGURATION (USING PROXY) ---
// We use window.location.origin to point to our own server's proxy
const getZamaConfig = () => {
  const origin = typeof window !== 'undefined' ? window.location.origin : '';
  return {
    kmsContractAddress: "0xbE0E383937d564D7FF0BC3b46c51f0bF8d5C311A",
    aclContractAddress: "0xf0Ffdc93b7E186bC2f8CB3dAA75D86d1930A433D",
    gatewayUrl: `${origin}/zama-gateway`, // Proxy handles CORS
    relayerUrl: `${origin}/zama-relayer`, // Proxy handles CORS
  };
};
    ...
    try {
      ...
      if (typeof window !== 'undefined' && window.ethereum) {
        ...
        if (accounts.length > 0) {
          ...
          // Init FHEVM with PROXY config
          try {
             const config = getZamaConfig();
             ...
             console.warn("FHEVM Init deferred (will retry on bid)", e);
          }
        }
      }
    } catch (e) { 
    ...
    try {
      ...
      if (!instance) {
        ...
        try {
          ...
          alert("Security Layer Failed. Check console for CORS/Network errors.");
          setIsLoading(false);
          return;
        }
      }
    ...
    <div className="min-h-screen bg-neutral-950 text-white font-sans selection:bg-yellow-500 selection:text-black flex flex-col relative">
          ...
          <div className="animate-in fade-in duration-500">
            <div className="text-center mb-16 space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 max-w-4xl mx-auto mt-12 bg-neutral-900/50 border border-neutral-800 rounded-2xl p-6 backdrop-blur-sm">
                ...
                <div className="text-center p-2 border-l border-neutral-800"><p className="text-3xl font-bold text-green-400">Online</p><p className="text-xs text-neutral-500 uppercase tracking-wider font-bold m-1">System Status</p></div>
              </div>
            </div>
      ...
      <footer className="border-t border-neutral-800 py-10 mt-12 bg-neutral-900/30">
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center text-sm text-neutral-500 gap-6">
          ...
          <a href="https://x.com/0xshinkai" target="_blank" rel="nopener noreferrer" className="flex items-center gap-2 hover:text-white transition-all duration-300 group cursor-pointer bg-neutral-800/50 px-4 py-2 rounded-full border border-neutral-700/50 hover:border-yellow-400/30 hover:bg-neutral-800">
            <span>Built by</span>
