...
// --- CONFIGURATION (edit .env.local values as needed) ---
...
// dynamic SDK loader: prefer @zama-fhe/relayer-sdk, fallback to fhevmjs
async function loadRelayerCreateInstance() {
  let sdk: any = null;
  try { sdk = await import('@zama-fhe/relayer-sdk'); } catch (e) { /* ignore */ }
  if (!sdk) {
    try { sdk = await import('fhevmjs'); } catch (e) { /* ignore */ }
  }
  if (!sdk) throw new Error('Relayer SDK not installed. Install @zama-fhe/relayer-sdk or fhevmjs.');
  const createInstance = sdk.createInstance ?? sdk.default?.createInstance;
  if (typeof createInstance !== 'function') throw new Error('createInstance not found on relayer SDK exports');
              ...
              <div className="flex flex-col justify-center space-y-8">
                ...
                <div className="bg-neutral-900 p-8 rounded-3xl border border-neutral-800 shadow-xl relative overflow-hidden">
                  ...
                  {selectedAuction.isActive ? (
                    ...
                    <div classNam="text-center py-8 bg-neutral-800/30 rounded-2xl border border-neutral-800/50"><Trophy className="w-12 h-12 text-yellow-400 mx-auto mb-3"/><h3 className="text-xl font-bold text-white">Auction Closed</h3><p className="text-neutral-500 text-sm">Winner is being calculated on-chain.</p></div>
                  )}
