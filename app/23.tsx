...
// SWITCHED TO RELAYER SDK (Make sure to npm install @zama-fhe/relayer-sdk)
...
export default function ShadowBidMarketplace() {
  ...
  const [fhevm, setFhevm] = useState<any>(null); // Using any to avoid type conflicts between libs
    ...
    try {
      ...
      if (typeof window !== 'undefined' && window.ethereum) {
        ...
        if (accounts.length > 0) {
          ...
          // Init Relayer SDK
          try {
             // The Relayer SDK handles the gateway/kms logic automatically for supported chains
             ...
             console.warn("SDK Init deferred", e);
          }
        }
      }
    } catch (e) { 
    ...
    try {
      ...
      if (!instance) {
        try {
          ...
          alert("Security Layer Failed. Check network connection.");
          setIsLoading(false);
          return;
        }
      }
      ...
      // SDK uses 'createEncryptedInput' same as base lib, but handles keys internally
            ...
            {auctions.length === 0 ? (
              <div className="py-20 border border-dashed border-neutral-800 rounded-3xl bg-neutral-900/10 text-center relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="text2xl font-bold text-white mb-2">No Active Auctions</h3>
