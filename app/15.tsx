...
// --- ZAMA SEPOLIA V0.9 CONSTANTS (CRITICAL FIX) ---
const ZAMA_KMS_ADDRESS = "0xbE0E383937d564D7FF0BC3b46c51f0bF8d5C311A";
const ZAMA_ACL_ADDRESS = "0xf0Ffdc93b7E186bC2f8CB3dAA75D86d1930A433D";
    ...
    try {
      ...
      if (typeof window !== 'undefined' && window.ethereum) {
        ...
        if (accounts.length > 0) {
          ...
          // Try to Init FHEVM
          try {
            // First attempt: Standard Init with Network URL
            ...
            setFhevm(instance);
             ...
             console.warn("Standard FHEVM init failed, trying with explicit KMS...");
             // Second attempt: Explicit KMS + ACL + Network URL
             try {
               const instance = await createInstance({ 
                 chainId: 11155111,
                 kmsContractAddress: ZAMA_KMS_ADDRESS,
                 aclContractAddress: ZAMA_ACL_ADDRESS,
                 networkUrl: READ_ONLY_RPC 
               });
               setFhevm(instance);
             } catch (retryErr) {
               console.error("FHEVM Init entirely failed", retryErr);
             }
          }
        }
      }
  ...
  const handleBid = async () => {
    ...
    try {
      ...
      if (!instance) {
        ...
        try {
          // ALWAYS use explicit KMS address AND Network URL
            ...
            <div className="grid md:grid-cols-2 gap-12">
              ...
              <div className="flex flex-col justify-center space-y-8">
                ...
                {account?.toLowerCase() === selectedAuction.creator.toLowerCase() && selectedAuction.isActive && (
                  <div className="grid grid-cols-2 gap-4">
                    <button onClick={() => handleEnd(false)} disabled={isLoading} className="bg-green-500/10 hver:bg-green-500/20 text-green-400 py-4 rounded-2xl font-bold border border-green-500/20 transition-all cursor-pointer hover:shadow-lg hover:shadow-green-500/10 active:scale-95">End Auction</button>
