...
// --- CONFIGURATION ---
// 1. Pointing to your Multi-Auction V2 Contract
...
export default function ShadowBidMarketplace() {
  ...
  const init = useCallback(async () => {
    try {
      let readProvider;
      // Use MetaMask if available, otherwise Public RPC (so images load for everyone)
      ...
      try {
        ...
        console.warn("Contract not found (Wrong network?).");
        return;
      }
      ...
      if (typeof window !== 'undefined' && window.ethereum) {
        ...
        // Only fetch balance/FHEVM if user is actually connected
  ...
  const connectWallet = async () => {
    ...
    try {
      ...
      console.error("Connection denied:", e); 
    }
  };
  ...
  const handleBid = async () => {
    ...
    try {
      ...
      // Convert Decimal to Integer (Scale by 100)
      // e.g., 0.05 ETH becomes 5 units in the contract
      ...
      // FORCE GAS LIMIT to bypass FHE estimation errors
